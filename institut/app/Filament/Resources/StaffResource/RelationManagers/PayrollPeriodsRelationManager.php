        ...
        return $table
            ...
            ->actions([
                ...
                Tables\Actions\Action::make('cancelAccrual')
                    ->label(__('general.cancel_accrual'))
                    ->icon('heroicon-o-x-circle')
                    ->color('danger')
                    ->requiresConfirmation()
                    ->modalHeading(__('general.cancel_accrual_confirm_title'))
                    ->modalDescription(__('general.cancel_accrual_confirm_desc'))
                    ->form([
                    ...
                    ->visible(fn (StaffPayrollPeriod $record): bool => in_array($record->status, ['approved', 'partially_paid'], true) && (auth()->user()?->hasAnyRole(['admin', 'accountant']) ?? false))
                    ->action(function (StaffPayrollPeriod $record, array $data): void {
                        ...
                        Notification::make()
                            ->title(__('general.accrual_cancelled_successfully'))
                            ->success()
                            ->send();
                    }),
            ]);
